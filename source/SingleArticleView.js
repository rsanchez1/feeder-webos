enyo.kind({
    ...
    components: [
        ...
        {name: "googleSharePopup", kind: "Menu", modal: false, dismissWithClick: true, components: [
            {name: "googleChromeShare", caption: "Share with Google", shareValue: "google", onclick: "chooseShare"},
            {caption: "Share with Twitter", shareValue: "twitter", onclick: "chooseShare"},
            {caption: "Share with Facebook", shareValue: "facebook", onclick: "chooseShare"},
        ]},
    ...
    shareClick: function(source, inEvent) {
        if (!!this.article.title) {
            ...
            if (!window.PalmSystem) {
                this.$.googleSharePopup.openAtEvent(inEvent);
            } else {
	...
	introText: "<div class='touchFeedsSummary'>" +
        ...
        (!!enyo.application.isPhone ? "<h2>Notice on Phones</h2>" +
